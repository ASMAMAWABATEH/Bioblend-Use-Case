...
def wait_for_job(gi, job_id, timeout=600, interval=5):
    """Poll a Galaxy job until it finishes or times out."""
    start = time.time()
    while True:
...
def upload_file_to_history(gi, file_path, history_id, file_type=FILE_TYPE, input_name="input1"):
    ...
    return upload["outputs"][0]["id"]


def run_tool(gi, history_id, tool_id=TOOL_ID, dataset_id=None, input_name="input1"):
    """Run a Galaxy tool using the dataset in the history"""
    ...
    return run["jobs"][0]["id"]


def show_history_contents(gi, history_id):
    """Return a list of datasets in the history"""
    outputs = gi.histories.show_history(history_id, contents=True)
    return [{"name": o["name"], "id": o["id"], "state": o.get("state", "unknown")} for o in outputs]


def main():
    """Execute full workflow: create history, upload, run tool, check job, show outputs"""
